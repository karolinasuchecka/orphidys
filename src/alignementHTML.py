                    ...
                    if synonyme_source == mot_target.attrib['lemma']:
                        ...
                        for synonyme_target in synonymes_target:
                            if synonyme_source == synonyme_target:
                                ...
                                print(type_correspondance, score_correspondances)
    if score_correspondances > 0.6 and mot_source.attrib['lemma'] == tete_groupe_source and mot_target.attrib['lemma'] \
            == tete_groupe_target:
        score_correspondances += 5
...
def texte_correspondant(id, fichier_corpus):
    dictionnaire_corpus = {}
    with open(fichier_corpus, newline='') as fichier_corpus:
        ...
        for ligne_corpus in fichier_corpus:
            dictionnaire_corpus[ligne_corpus[0]] = ligne_corpus[1].lower()

    return (dictionnaire_corpus[id])

def remplacement_resultats(dictionnaire_phrases_remplacer, phrases_texte, texte, fichier_corpus):
    for id, apparat in dictionnaire_phrases_remplacer.items():
        texte_phrase = texte_correspondant(id, fichier_corpus)
        texte_phrase = re.sub(r"\? …", r"?…", texte_phrase)
        texte_phrase = re.sub(r"! …", r"!…", texte_phrase)
        texte_phrase = re.sub(r"\xA0", " ", texte_phrase)
        print("texte phrase", texte_phrase)
        leven_gagnant = [500, '']

        for phrase in phrases_texte:
            phrase_de_comparaison = re.sub(r'<.+?>',"", phrase)
            phrase_de_comparaison = re.sub(r"\s+", " ", phrase_de_comparaison)
            phrase_de_comparaison = re.sub(r"\t+?", " ", phrase_de_comparaison)
            phrase_de_comparaison = re.sub(r"\n+?", " ", phrase_de_comparaison)
            phrase_de_comparaison = phrase_de_comparaison.lower()
            leven_score = Levenshtein.distance(phrase_de_comparaison, texte_phrase)
            if leven_score < leven_gagnant[0]:
                leven_gagnant[0] = leven_score
                leven_gagnant[1] = phrase


        print("Gagnant", leven_gagnant[1], leven_gagnant[0])

        if apparat != '' and leven_gagnant[1] != '':
            texte_a_remplacer = ET.tostring(apparat, encoding='utf8').decode()
            print(texte_a_remplacer)
            texte = re.sub(re.escape(leven_gagnant[1]), texte_a_remplacer, str(texte))
        else:
            texte = re.sub(re.escape(leven_gagnant[1]), '', str(texte))
    return (texte)
    ...
    for texte in textes:
        ...
        for phrase in phrases:
            ...
            if id_phrase not in phrases_traites:
                phrases_communes = texte.findall(
                    ".//s[@{http://www.w3.org/XML/1998/namespace}id = '" + id_phrase + "']")
                id_corresps = ''
                ...
                if id_phrase not in id_phrases.keys():
                    ...
                    for corresp in corresps:
                        ...
                        if score_correspondance_groupes >= 10:
                            ...
                            for mot_source in mots_source:
                                ...
                                if mot_source.text.lower() not in traitementsTal.stopwords:
                                    target_gagnant = trouve_target_gagnant(mot_source, mots_target, rootResults,
                                                                           id_phrase, corresp)
                                    target_gagnant.insert(0,
                                                          mot_source.attrib['{http://www.w3.org/XML/1998/namespace}id'])
                    ...
                    for phrase_retenue in phrases_correspondantes_scores.keys():
                        ...
                        rdg = ET.Element('rdg', attrib={'xml:id': phrase_retenue,
                                                        "source": trouve_auteur(phrase_retenue, rootResults)})
                        rdg.append(
                            rootResults.find(
                                ".//s[@{http://www.w3.org/XML/1998/namespace}id = '" + phrase_retenue + "']"))
                        app.append(rdg)
                        for resultat in resultats:
                            ...
                            w_source = phrase.find(
                                ".//w[@{http://www.w3.org/XML/1998/namespace}id = '" + id_source + "']")
        ...
        texte_src = ET.tostring(texte_root, encoding='utf8').decode()
        texte_src = re.sub(r"<\?xml version='1\.0' encoding='utf8'\?>", r"", str(texte_src))
        texte_src = re.sub(r"\.", r". ", texte_src)
        texte_src = re.sub(r"\?", r"? ", texte_src)
        texte_src = re.sub(r"\? …", r"?…", texte_src)
        ...
        texte_src = remplacement_resultats(remplacements, phrases_txt, texte_src, traitement.fichier_corpus)
