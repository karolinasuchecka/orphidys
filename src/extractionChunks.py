...
def recherche_groupes_verbaux(phrase, mots):
    ...
    composants_GV = ['ADV', 'PRO:REL', "PRO:PER","PRO:DEM", "PRP:det", 'PRP']
    groupes_verbaux = {}
    ...
    while i < identifiants[len(identifiants)-1]:
        if i in mots.keys() and mots[i].tag == 'w' and mots[i].attrib['pos'] in temps_verbaux:
            ...
            #print(texte_phrase)
            #print('###')
            if i-1 in mots.keys() and mots[i-1].tag == 'w' and (mots[i-1].attrib['pos'] in composants_GV or mots[i-1].attrib['pos']
            == 'VER:infi'):
                if i - 2 in mots.keys() and mots[i - 2].tag == 'w' and mots[i - 2].attrib['pos'] in composants_GV:
                    if i-3 in mots.keys() and mots[i-3].tag == 'w' and mots[i-3].attrib['pos'] in composants_GV:
            ...
            if i +1 in mots.keys() and mots[i+1].tag == 'w' and (mots[i+1].attrib['pos'] in composants_GV or mots[i+1].attrib['pos'] in autres_formes) and \
                mots[i + 1].attrib['pos'] != mots[i].attrib['pos']:
                #print (mots[i+1].text, mots[i+1].attrib['pos'])
                groupes_verbaux[id_groupe].append(mots[i+1].attrib['n'])
                if i +2 in mots.keys() and mots[i+2].tag == 'w' and (mots[i+2].attrib['pos'] in composants_GV or mots[i+2].attrib['pos'] in autres_formes):
                    #print (mots[i+2].text, mots[i+2].attrib['pos'])
                    groupes_verbaux[id_groupe].append(mots[i + 2].attrib['n'])
            #print('###')
        i += 1
...
def recherche_groupes_adjectivaux (phrase, mots):
    identifiants = sorted(mots.keys())
    i = identifiants[0]
    constituants = ['ADV', 'PRO:DEM', 'DET:ART', 'PRO:PER', 'PRP', "PRO:REL"]
    groupes_adjectivaux = {}
    while i < identifiants[len(identifiants) - 1]:
        if i in mots.keys() and mots[i] != None and mots[i].tag == 'w' and (mots[i].attrib['pos'] == 'ADJ' or mots[i].attrib['pos'] == 'VER:pper'):
            #print('PHRASE : ')
            texte_phrase = ''
            for elem in phrase.iter():
                if elem.text != None:
            ...
            if i-1 in mots.keys() and mots[i-1] != None  and mots[i-1].tag == 'w' and mots[i-1].attrib['pos'] in constituants:
                if i - 2 in mots.keys() and mots[i-2] != None  and mots[i - 2].tag == 'w' and mots[i-2].attrib['pos'] in constituants:
                    #print(mots[i-2].text, mots[i-2].attrib['pos'])
            ...
            if i+1 in mots.keys() and mots[i+1] != None  and mots[i+1].tag == 'w' and mots[i+1].attrib['pos'] in constituants:
                #print(mots[i+1].text, mots[i+1].attrib['pos'])
                groupes_adjectivaux[str(i)].append(str(i + 1))
                if i + 2 in mots.keys() and mots[i+2] != None and mots[i + 2].tag == 'w' and mots[i+1].attrib['pos'] in constituants:
                    #print(mots[i+2].text, mots[i+2].attrib['pos'])
                    groupes_adjectivaux[str(i)].append(str(i + 2))
...
def extraction_chunks(fichier, dossier, racine):
    ...
    for phrase in rootResults.findall(".//s"):
        ...
        for mot in mots:
            if int(mot.attrib['n']) > n_courant:
                ...
                #GADJ
                elif id in groupes_adjectivaux.keys() and phrase.find('w[@n = \''+id+'\']') != None:
                    mot_tete = phrase.find('w[@n = \'' + id + '\']').attrib["lemma"]
    ...
    treeResults.write('resultat_avec_chunks.xml')
    os.chdir(racine)
