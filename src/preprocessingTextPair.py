...
def initialisation_text_pair (traitement, racine):
    ...
    #Suppression des éléments peu pertinents
    preparation_textes = (input('Traiter les XML ? (O/N ) '))
    if preparation_textes == 'O':
        liste_xml = os.listdir(traitement.dossier_source)
        if not os.path.exists("XML_"+nom_projet):
            os.makedirs("XML_"+nom_projet)
        for xml in liste_xml:
            nom_fichier = xml[:-4]
            print("java -jar src/saxon9he.jar -xsl:src/xml2xml_textpair.xsl -s:" + traitement.dossier_source + "/"
                      + nom_fichier + ".xml -o:"+racine+'/XML_'+nom_projet+'/'+nom_fichier+".xml")
            os.system("java -jar src/saxon9he.jar -xsl:src/xml2xml_textpair.xsl -s:" + traitement.dossier_source + "/"
                      + nom_fichier + ".xml -o:"+racine+'/XML_'+nom_projet+'/'+nom_fichier+".xml")
        traitement.dossier_source = racine+'XML_'+nom_projet
        traitement.dossier_target = racine + 'XML_' + nom_projet
    ...
    if lemmatisation == 'O':
        ...
        for ligne in liste_lemmes.values():
            fichier_lemmes.write(ligne[0] + '\t' + ligne[1] + '\n')
